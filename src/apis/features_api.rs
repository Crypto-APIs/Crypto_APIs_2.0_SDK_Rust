...
pub enum BroadcastLocallySignedTransactionError {
    ...
    Status409(crate::models::InlineResponse40924),
...
/// struct for typed errors of method `convert_bitcoin_cash_address`
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ConvertBitcoinCashAddressError {
...
pub async fn convert_bitcoin_cash_address(configuration: &configuration::Configuration, blockchain: &str, network: &str, context: Option<&str>, convert_bitcoin_cash_address_rb: Option<crate::models::ConvertBitcoinCashAddressRb>) -> Result<crate::models::ConvertBitcoinCashAddressR, Error<ConvertBitcoinCashAddressError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/blockchain-tools/{blockchain}/{network}/address/convert", configuration.base_path, blockchain=crate::apis::urlencode(blockchain), network=crate::apis::urlencode(network));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
    ...
    local_var_req_builder = local_var_req_builder.json(&convert_bitcoin_cash_address_rb);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ConvertBitcoinCashAddressError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
...
pub async fn derive_hd_wallet__x_pub_y_pub_z_pub_change_or_receiving_addresses(configuration: &configuration::Configuration, blockchain: &str, extended_public_key: &str, network: &str, context: Option<&str>, address_format: Option<&str>, addresses_count: Option<i64>, is_change: Option<bool>, start_index: Option<i64>) -> Result<crate::models::DeriveHdWalletXPubYPubZPubChangeOrReceivingAddressesR, Error<DeriveHdWalletXPubYPubZPubChangeOrReceivingAddressesError>> {
